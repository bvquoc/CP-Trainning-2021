...
const string FNAME = "GENTEST"; // Ten chuong trinh
...
void generateTestCase(string f_inp) {
	...
	*/
	...
	ll n = 1e9; // mrand(20,500); // tao so n ngau nhien tu [10, 1000000]
	ll m = 1000; //mrand(2, n * (n - 1) / 2);
	inp << n << ' ' << m << endl; // ghi so n ra file *.INP
	int k = m;
	for (int i = 1; i <= m; i++) {
		int x = mrand(1,k);
		inp << x << ' ';
