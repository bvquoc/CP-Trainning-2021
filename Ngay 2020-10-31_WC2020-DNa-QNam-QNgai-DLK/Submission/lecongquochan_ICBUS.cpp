...
int n, m;
int cost[limit], maxDist[limit];
int dist[5005][5005];
vi g[limit];

void dijkstra() {
    pq <Data, vector <Data>, CMP> heap;
    REP(i, 1, n) REP(j, 0, n) dist[i][j] = INF;
    dist[1][0] = 0;
    heap.push({1, 0, 0});
    ...
    while (!heap.empty()) {
        ...
        int remain = heap.top().remain;
        heap.pop();

        //ticket can be used
        if (remain) {
            for (auto v : g[u]) {
                if (dist[v][remain - 1] > curDist) {
                    dist[v][remain - 1] = curDist;
                    heap.push({v, dist[v][remain - 1], remain - 1});
                }
            }
        }

        //buy new ticket
        if (dist[u][maxDist[u]] > curDist + cost[u]) {
            dist[u][maxDist[u]] = curDist + cost[u];
            heap.push({u, dist[u][maxDist[u]], maxDist[u]});
        }
    }
}
...
void solveProblem() {
    dijkstra();
    int ans = INF;
    REP(i, 0, n) ans = min(ans, dist[n][i]);
    cout << ans << endl;
}
