...
void Calc(){
    ...
    while (ntest--){
        ...
        int ans = n;
        reset(root);
        for (int i = 1; i <= cntback && aback[i] > r; i++){
            int p = get_root(a[aback[i]].F);
            int q = get_root(a[aback[i]].S);
            if (p - q){
                root[p] = q;
                ans--;
            }
        }
        for (int i = 1; i <= cntgo && ago[i] < l; i++){
            int p = get_root(a[ago[i]].F);
            int q = get_root(a[ago[i]].S);
            if (p - q){
                root[p] = q;
                ans--;
            }
