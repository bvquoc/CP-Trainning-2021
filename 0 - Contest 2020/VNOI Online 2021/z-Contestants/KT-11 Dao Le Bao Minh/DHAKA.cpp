...
// supporter
i32 melon[N + 1];
vector<pair<i32, i64>> adj[N + 1];
i64 path[N + 1][128];
struct Node {
	i32 u, melon; i64 w;
	Node(i32 u, i64 w, i32 melon) : u(u), w(w), melon(melon) {}
	friend bool operator>(Node a, Node b) {
		return bit1(a.melon) > bit1(b.melon) || (bit1(a.melon) == bit1(b.melon) && a.w > b.w);
	}
	tuple<i32, i64, i32> operator()() {
		return make_tuple(u, w, melon);
	}
};
...
i32 main() {
	...
	for (i32 i : range(128))
		if (bit1(i >> 1) >= l)
			minimize(ans, path[n][i]);
	write(ans != MAX64 ? ans : -1);
...
void dijkstra(i32 s) {
	for (i32 i : range(1, n + 1))
		for (i32 j : range(0, 128))
			path[i][j] = MAX64;
	heapmin<Node> pq;
	pq.emplace(s, 0ll, melon[s]);
	path[s][melon[s]] = 0ll;
	while (pq.empty() == false) {
		i32 u, mask; i64 uLen; tie(u, uLen, mask) = Node(pq.top())();
		pq.pop();
		...
		for (pair<i32, i64> i : adj[u]) {
			i32 v; i64 vLen; tie(v, vLen) = i;
			if (uLen + vLen < path[v][mask | melon[v]])
				path[v][mask | melon[v]] = uLen + vLen,
				pq.emplace(v, uLen + vLen, mask | melon[v]);
		}
	}
}
