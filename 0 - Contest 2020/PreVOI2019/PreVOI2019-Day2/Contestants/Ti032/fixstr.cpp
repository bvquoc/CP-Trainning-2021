    ...
    if(n<=1000 && m<=1000)
    {
        ...
        while(m--)
        {
            ...
            if(c=='C')
            {
                int k;
                cin>>k;
                for(int i=1;i<=n;i++) b[dem][i]=b[dem-1][i];
                if(b[dem][k]==-1) b[dem][k]=1;
                else b[dem][k]=-1;
            }
            else
            if(c=='Q')
            {
                for(int i=1;i<=n;i++) b[dem][i]=b[dem-1][i];
                sum[0]=0;
                for(int i=1;i<=n;i++) sum[i]=sum[i-1]+b[dem][i];
                int l,r;
                cin>>l>>r;
                int mi=mod;
                for(int i=l;i<=r;i++) mi=min(mi,sum[i]-sum[l-1]);
                cout<<sum[r]-sum[l-1]-mi+abs(mi)<<endl;
            }
            else
            {
                int k;
                cin>>k;
                for(int i=1;i<=n;i++) b[dem][i]=b[k-1][i];
            }
