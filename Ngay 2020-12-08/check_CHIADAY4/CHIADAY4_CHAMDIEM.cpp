/*
* Chuong trinh cham diem cho code cua ban ^.^
* Author: Tran Huu Nhiem
* Facebook: Nhiem HK
*/
...
const string FNAME = "CHIADAY4"; // Ten chuong trinh
const ll N_TEST = 20; // So luong test case can kiem tra
...
void generateTestCase(string f_inp) {
	...
	/*
	Ban can chinh sua ham nay de tao file *.INP theo de bai o day nhe!!!
	
	Vi du:
	
	ll n = mrand(1e1, 1e6); // tao so n ngau nhien tu [10, 1000000]
	inp << n << endl; // ghi so n ra file *.INP
	
	*/
	
	ll n = 300; // mrand(20,500); // tao so n ngau nhien tu [10, 1000000]
	inp << n << endl; // ghi so n ra file *.INP

	for (int i = 1; i <= n; i++) {
		int x = mrand(-1000000,1000000);
		inp << x << ' ';
	}
	inp.close();
}
