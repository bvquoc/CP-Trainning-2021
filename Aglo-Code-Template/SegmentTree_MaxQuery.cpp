/* Author: Bui Vi Quoc */
// https://lqdoj.edu.vn/problem/querymax
#include <bits/stdc++.h>
...
struct SegmentTree {
    ...
    int getMax(int i, int l, int r, int u, int v) {
        ...
        pushDown(i);
        int m = (l + r) >> 1;
        int L = getMax(i << 1, l, m, u, v);
        int R = getMax((i << 1) + 1, m + 1, r, u, v);
        return max(L, R);
    }

	void update(int u, int v, int c) { update(1, 1, n, u, v, c); }
	int getMax(int u, int v) { return getMax(1, 1, n, u, v); }
